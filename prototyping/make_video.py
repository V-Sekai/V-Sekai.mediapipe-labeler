...
class FileStruct:
    ...
    def add_wave(self, wave_file):
        cmd_id = os.path.splitext(os.path.basename(wave_file))[0].split('_')[4]  
    ...
    def add_png(self, png_file):
        cmd_id = os.path.splitext(os.path.basename(png_file))[0].split('_')[4]  
...
def main(root_dir):
    ...
    for dir in glob.glob(os.path.join(root_dir, 'sub_*_ia')):
        ...
        for trial_dir in glob.glob(os.path.join(dir, 'trial_*')):
            ...
            all_files = mic_files_1 + mic_files_2 + png_files_rgb
